...
라고 표현한다. 여기서, $t$는 frame index를 의미하고, $v$는 viewpoint를 의미한다. 그러니, $P_{t,v}$는 시간 $t$, viewpoint $v$에서 얻은 colored 3D point cloud라고 생각하면 된다.
...
처럼 depth를 이용해 3D 위치를 복원할 수 있고, 여기에 원래 RGB color를 붙이면 colored point cloud가 된다. 이렇게 만든 point cloud들을 시간과 viewpoint에 따라 저장해 둔 것이 spatiotemporal 3D Cache $P_{t,v}$다. 논문에서는 이를 $L \times V$ 구조로 설명한다:
...
이는 video diffusion model이 장면 구조를 순전히 기억하도록 맡기는 것이 아니라, 외부에 3D 형태의 scene memory를 만들어 둔 것이라고 이해하면 편하다.
...
그런데, 여기서 문제가 있다. single-view point cloud의 경우에는 앞에 물체가 있을 경우 물체 뒷면까지 채워넣지 않는다. 그래서 만약 camera view가 달라졌을 때 물체 뒤의 영역이 드러날 경우, 해당 영역이 비워있을 수 있다. 이럴 경우, 우리는 이를 **disocclusion**이라고 표현한다.
...
이를 **structured guidance로 video diffusion model**에 제공한다. 그러면 video diffusion model은 3D cache로부터 알려진 영역은 최대한 유지하고 M으로 표시된 missing region은 생성해서 채우고, camera trajectory에 맞춰 연속적인 video를 생성한다.
...
가 된다. 이렇게 하면 Plücker latent channel이 $2C$ 가 되는데, 반면 video latent $Z$는 $C$이므로, lightweight MLP를 이용해서 $C$로 줄인다.
...
여기까지 우리는 $Z,E$를 준비를 했다. 그런데, 이걸 바로 Transformer에 넣지 않고, **Patchification**이라는 방식을 추가적으로 사용한다. $Z$와 $E$ 각각을 먼저 $2 \times 2$ patchification layer만 사용해서 video latent를 reconstruction network가 처리할 flattened token으로 만든다.
...
의 **per-pixel 3D Gaussian features**라고 표현한다. 여기서 왜 14개인지 확인하자면, 
...
에 해당하는 depth supervision을 획득하고, 이를 $D_{student}$와 비교한다. 여기서 단순히 depth MSE가 아니라 Scale-Invariant depth loss를 사용하는데, 이는 Long-LRM의 scale-invariant depth loss를 사용한다고 한다. 그래서, absolute depth scale 자체보다는 상대적인 depth structure를 맞추게 된다. 즉, depth 값을 정확하게 맞추는 것이 아니라 depth 구조를 맞추도록 돕는다.
...
그런데, RGB VAE encoder는 3-channel input을 기대한다. 그래서 time scalar 1개에 2-dimensional sinusoidal embedding을 붙여 3 channel로 만든다.
...
로 video latent와 동일한 형태가 된다. Target time은 latent temporal dimension $L'$ 전체에 repeat 된다. 3DGS decoder architecture figure를 보면 바로 이해할 수 있다.
