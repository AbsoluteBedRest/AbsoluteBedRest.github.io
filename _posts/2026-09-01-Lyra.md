...
1. multi-view dataset 필요없이 학습하는 self-distillation framework를 구축했다.
2. dynamic scene까지 확장 가능한 framework를 구축했다.
3. 다양한 scene에서 그리고 single-image to 3D scene genration, single-image to 4D scene generation 에서 SOTA 결과를 달성했다.
...
향후 연구 방향으로는 
- 더 강략한 video diffusion model을 framework에 접목시키는 것
